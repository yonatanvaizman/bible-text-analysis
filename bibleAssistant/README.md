...
- [evaluation.py](evaluation.py): A module for evaluating an agent.
...
- ![WIP][WIP] Evaluation: semi-automate evaluating using ground-truth "golden" examples. At each LLM point check if the LLM did the expected thing (call the right tool, with right args, respond correctly to user). This will be useful for training a rigid behavior, but will become less relevant once I want my agent to be autonomous and creative (once there will be many possible ways to solve a problem). [eval_agent.ipynb](eval_agent.ipynb)
